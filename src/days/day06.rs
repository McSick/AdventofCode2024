...
fn solve1(mut map: Map) -> usize {
    ...
    map.grid.iter().flat_map(|row| row.iter()).filter(|tile| tile.visited).count()
 }
...
impl Map {
    ...
    fn is_edge(&self, location: (usize, usize)) -> bool {
        location.0 == 0 || location.1 == 0 || location.0 == self.grid.len() - 1 || location.1 == self.grid[0].len() - 1
    }
    fn try_move_guard_out(&mut self) -> State {
        ...
        if self.will_hit_obstacle() {
            self.turn_guard();
            if self.visited_positions.contains(&(self.guard.location.0, self.guard.location.1, self.guard.direction.clone())) {
                return State::Looping;
            } else {
                self.visited_positions.insert((self.guard.location.0, self.guard.location.1, self.guard.direction.clone()));
            }
            return State::Moving;
        }
        self.move_guard();
    ...
    fn will_hit_obstacle(&self) -> bool {
        match self.guard.direction {
            Direction::Up => self.grid[self.guard.location.0 - 1][self.guard.location.1].tile_type == TileType::Obstacle,
            Direction::Right => self.grid[self.guard.location.0][self.guard.location.1 + 1].tile_type == TileType::Obstacle,
            Direction::Down => self.grid[self.guard.location.0 + 1][self.guard.location.1].tile_type == TileType::Obstacle,
            Direction::Left => self.grid[self.guard.location.0][self.guard.location.1 - 1].tile_type == TileType::Obstacle
...
impl Tile {
    fn new(c: char) -> Tile {
        ...
        Tile { tile_type: TileType::from_char(c), visited: visited }
    }
}
...
fn read_input(file_name: &str) -> Map {
    ...
    for (i, line) in text.lines().enumerate() {
        ...
        for (j, c) in line.chars().enumerate() {
            ...
            match c {
                '^'=>  some_guard = Some(Guard { location: (i, j), direction: Direction::Up }),
                'v' =>  some_guard = Some(Guard { location: (i, j), direction: Direction::Down }),
                '<' => some_guard = Some(Guard { location: (i, j), direction: Direction::Left }),
                '>' =>  some_guard = Some(Guard { location: (i, j), direction: Direction::Right }),
                _ => ()
            }
        }
    }
    ...
    visited_positions.insert((guard.clone().location.0, guard.clone().location.1, guard.clone().direction));
    Map { grid: grid, guard: guard, visited_positions: visited_positions }
}
