...
mod tests {
    ...
    fn test_find_mul_ops() {
        assert_eq!(find_mul_ops("mul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))"), 161);
    }
    #[test]
    fn test_find_mul_ops_do_or_do_not() {
        assert_eq!(find_mul_ops_do_or_do_not("xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))"), 48);
    }
}
